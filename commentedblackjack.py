...
#hand is parameter value which is a int
def calcHandValue(hand):
    total = 0
    for h in hand:
        #picks the name out of the list of lists, "h"
        name = h[1]
        if name == "Ace":
            #adds11, and if over 11, it changes to a 1
            total = total + 11
            if total > 21:
                total = total - 10
...
def main():
    ...
    #while true keeps the code repeating till the block hits break
    while True:
        option = input("Type 1: Hit  or 2: Stand\n")
        if option == "1":
            #adds a card to the list of "userHand"
